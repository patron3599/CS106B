...
private:
    ...
    void checkResize(); // Double the array size if it's at capacity
    bool verifyIndex(int index); // Ensure that we are not accessing memory outside the array
    void validateIndex(int index); // Ensure that we are not accessing memory outside the array

    /* * Provided Variables and Methods* */
